...
export const listProducts = async (req, res) => {
  try {
    ...
    const products = await Product.find(findQuery)
      .select("name price image images category subcategory section status displayDescription isHotSelling isCreateHype createdAt")
      .sort({ createdAt: -1 })
      .limit(limitNum)
      .lean();

    // Browser/CDN cache — homepage can reuse responses while scrolling/revisiting
